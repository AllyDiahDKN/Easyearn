...
if (isset($_GET['brand_id'])) {
    ...
    $sql = "SELECT * FROM deleted_brands WHERE brand_id = '$brandId'";
    ...
    if ($result->num_rows > 0) {
        ...
        $insertQuery = "INSERT INTO brands (brand_id, brand_name)
                        VALUES ('{$brandData['brand_id']}', '{$brandData['brand_name']}')";

        if ($conn->query($insertQuery) === TRUE) {
            // Delete the brand from the brands table
            $deleteQuery = "DELETE FROM deleted_brands WHERE brand_id = '$brandId'";
