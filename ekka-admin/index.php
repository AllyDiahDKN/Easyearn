...
?>	
						...
						<div class="col-xl-3 col-sm-6 p-b-15 lbl-card">
							<div class="card card-mini dash-card card-2">
								<div class="card-body">
									<?php
$sql = "SELECT COUNT(*) as total_products FROM products WHERE availability = '1'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  $row = $result->fetch_assoc();
  $total_products = $row['total_products'];
    echo '<h2 class="mb-1">'.$total_products.'</h2>';
} else {
  echo '<h2 class="mb-1">0</h2>';
}
?>	
						...
						<div class="col-xl-3 col-sm-6 p-b-15 lbl-card">
							<div class="card card-mini dash-card card-3">
								<div class="card-body">
									<?php
$sql = "SELECT COUNT(*) as total_orders FROM orders WHERE Payment = 'paid'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  $row = $result->fetch_assoc();
  $total_orders = $row['total_orders'];
    echo '<h2 class="mb-1">'.$total_orders.'</h2>';
} else {
  echo '<h2 class="mb-1">0</h2>';
}
?>										<p>Weekly Orders</p>
